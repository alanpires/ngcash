...
export const createTransactionService = async (
  ...
  const findAccountCashOut = (await AccountRepository.findByIdWhithoutRelations(
    accountId,
  )) as Account;
  ...
  const findAccountCashIn = (await AccountRepository.findByUser(
    findUserCashIn,
  )) as Account;
...
export const filterTransactionService = async (
  accountId: string,
  start_date: string,
  end_date: string,
  cashIn?: boolean,
  ...
  const account = (await AccountRepository.findByIdWhithoutRelations(
    accountId,
  )) as Account;

  const { startDateConverted, endDateConverted } = convertedDateService(
    start_date,
    end_date,
  );

  const transactionsCashOut = await TransactionRepository.findCashOutByDate(
    account,
    startDateConverted,
    endDateConverted,
  );
  const transactionsCashIn = await TransactionRepository.findCashInByDate(
    account,
    startDateConverted,
    endDateConverted,
  );
